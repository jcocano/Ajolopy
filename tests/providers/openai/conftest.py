"""Shared fixtures and helpers for OpenAIProvider tests.

The tests *never* hit the network — every interaction with the OpenAI SDK
goes through ``AsyncMock`` shims set up here. ``OPENAI_API_KEY`` is cleared
per-test so the provider's env-var fallback branch is reachable
deterministically; opt back in with ``with_openai_key``.
"""
...
def ensure_provider_registered_and_clean_env(
    ...
    # every test, so we must re-register OpenAIProvider here for tests in
...
def with_openai_key(monkeypatch: pytest.MonkeyPatch) -> str:
    """Set a fake OPENAI_API_KEY for the duration of the test."""
    fake = "sk-test-only"
...
def make_chat_completion(
    *,
    ...
    OpenAI's real Pydantic models expose ``choices[0].message.{content,
    tool_calls}``, ``choices[0].finish_reason``, and ``usage.{prompt_tokens,
    completion_tokens}``. We only need these attributes for the conversion
    helpers in OpenAIProvider, so a SimpleNamespace tree is enough.
    """
...
def make_stream_chunk(
    *,
    ...
    The relevant fields are ``choices[0].delta.{content, tool_calls}`` and
    ``choices[0].finish_reason``. Mirrors OpenAI's streaming wire shape.
    """
    raw_tool_calls: list[SimpleNamespace] = []
...
class _AsyncStreamIterator:
    ...
    Tracks ``close_called`` so tests can assert the provider cancels the
    underlying SDK stream when the caller bails out early.
    """
...
def make_async_client(
    *,
    ...
    ``chat.completions.create`` is an ``AsyncMock``; when called with
    ``stream=True`` it resolves to an async iterator over ``stream_events``,
    otherwise it returns ``create_return``.
    ``embeddings.create`` likewise is an ``AsyncMock``.
    """
    ...
    # Expose the stream iterator on the mock so tests can introspect close().
