"""Errors raised by ``OpenAIProvider``.

All provider-specific errors derive from ``OpenAIProviderError`` so callers
...
class OpenAIProviderError(LLMProviderError):
    """Base class for any error raised inside ``OpenAIProvider``."""


class OpenAIConfigError(OpenAIProviderError):
    """The provider could not be constructed (e.g. missing ``OPENAI_API_KEY``)."""
