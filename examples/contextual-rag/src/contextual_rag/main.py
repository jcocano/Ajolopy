...
- ``app`` — the underlying Starlette ASGI3 callable, suitable for bare
  uvicorn (``uvicorn contextual_rag.main:app``). The wrapper itself
...
``on_app_bootstrap`` / ``on_app_shutdown`` hooks. The contextual-rag
...
# ``contextual_rag.main:app`` (the Starlette inner app) directly.
...
def main() -> None:
    ...
    # hypercorn, etc.) mount ``contextual_rag.main:app`` directly
