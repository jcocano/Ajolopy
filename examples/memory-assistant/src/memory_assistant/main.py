...
- ``app`` — the underlying Starlette ASGI3 callable, suitable for bare
  uvicorn (``uvicorn memory_assistant.main:app``). The wrapper itself
...
``on_app_bootstrap`` / ``on_app_shutdown`` hooks. The memory-assistant
...
# ``memory_assistant.main:app`` (the Starlette inner app) directly.
...
def main() -> None:
    ...
    # hypercorn, etc.) mount ``memory_assistant.main:app`` directly
