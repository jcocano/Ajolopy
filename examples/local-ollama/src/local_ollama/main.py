...
- ``app`` — the underlying Starlette ASGI3 callable, suitable for bare
  uvicorn (``uvicorn local_ollama.main:app``). The wrapper itself
...
``on_app_bootstrap`` / ``on_app_shutdown`` hooks. The local-ollama
...
# ``local_ollama.main:app`` (the Starlette inner app) directly.
...
def main() -> None:
    ...
    # hypercorn, etc.) mount ``local_ollama.main:app`` directly without
    # invoking ``main()``.
