...
- ``app`` — the underlying Starlette ASGI3 callable, suitable for bare
  uvicorn (``uvicorn support_agent.main:app``). The wrapper itself
...
``on_app_bootstrap`` / ``on_app_shutdown`` hooks. The support-agent
...
from support_agent.app_module import AppModule
