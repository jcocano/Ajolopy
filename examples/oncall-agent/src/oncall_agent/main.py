...
- ``app`` — the underlying Starlette ASGI3 callable, suitable for bare
  uvicorn (``uvicorn oncall_agent.main:app``). The wrapper itself
...
``on_app_bootstrap`` / ``on_app_shutdown`` hooks. The oncall-agent
...
# ``oncall_agent.main:app`` (the Starlette inner app) directly.
...
def main() -> None:
    ...
    # hypercorn, etc.) mount ``oncall_agent.main:app`` directly without
    # invoking ``main()``.
